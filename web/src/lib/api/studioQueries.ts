// TanStack Query hooks + query keys for the studio (ported from
// studio-web/src/api/queries.ts).
//
// Query keys:
//   ['datasets']
//   ['dataset', id]
...
//   rollback -> invalidate ALL keys for the id plus ['datasets'].
...
import * as api from './studioClient';
import type {
  AddLineBody,
  CaptureParams,
  CreateDatasetBody,
...
export function useRollback() {
  const qc = useQueryClient();
  return useMutation<DatasetDetail, Error, { datasetId: string; targetRevisionId: number }>({
    mutationFn: ({ datasetId, targetRevisionId }) => api.rollback(datasetId, targetRevisionId),
    onSuccess: (_result, { datasetId }) => {
